...
public class GmlUtils {
	 ...
	 */
	public static TemporalGeometricPrimitive gmlToTemporalGeometricPrimitive(
			Element gmlTime) {
		...
		if (gmlTime.getLocalName().equals("TimeInstant")) {
			Element timePosition = (Element) gmlTime.getElementsByTagNameNS(
					GML_NS, "timePosition").item(0);
			if (!timePosition.getAttribute("frame").isEmpty()) {
				frame = timePosition.getAttribute("frame");
			}
			if (frame.isEmpty() || frame.contains("8601")) {
				try {
					instants.add(ZonedDateTime.parse(
							timePosition.getTextContent(),
							DateTimeFormatter.ISO_DATE_TIME));
				} catch (DateTimeParseException dtpe) {
					// TODO: could be date, gYearMonth, gYear
				}
			} else {
				throw new RuntimeException(
						"Unsupported temporal reference frame: " + frame);
			}
		} else { // gml:TimePeriod
			Element beginPosition = (Element) gmlTime.getElementsByTagNameNS(
					GML_NS, "beginPosition").item(0);
			instants.add(ZonedDateTime.parse(beginPosition.getTextContent(),
					DateTimeFormatter.ISO_DATE_TIME));
			Element endPosition = (Element) gmlTime.getElementsByTagNameNS(
					GML_NS, "endPosition").item(0);
			instants.add(ZonedDateTime.parse(endPosition.getTextContent(),
					DateTimeFormatter.ISO_DATE_TIME));
		}
		TemporalFactory tmFactory = new DefaultTemporalFactory();
		TemporalGeometricPrimitive timePrimitive = null;
		if (instants.size() == 1) {
			timePrimitive = tmFactory.createInstant(new DefaultPosition(Date
					.from(instants.get(0).toInstant())));
		} else {
			Instant beginInstant = tmFactory.createInstant(new DefaultPosition(
					Date.from(instants.get(0).toInstant())));
			Instant endInstant = tmFactory.createInstant(new DefaultPosition(
					Date.from(instants.get(1).toInstant())));
